use bevy::{
    app::{Plugin, Update},
    ecs::{
        entity::Entity, event::EventReader, system::{Commands, Query, Res}
    },
    log::debug, time::{Fixed, Time},
};

use crate::{
    map::{HexGrid, HEX_RADIUS_IN_METERS},
    movement::{GamePosition, Kph, MoveUnitEvent, MovementConfig, MovingTowards, Path, PROGRESS_COMPLETE, PROGRESS_ZERO},
};
...
const TEMPORARY_MOVE_SPEED: Kph = 30.0;

fn move_unit_along_path(
    mut query: Query<(Entity, &mut MovingTowards, &mut Path)>,
    time: Res<Time<Fixed>>,
    mut commands: Commands,
) {
    for (entity, mut moving, mut path) in query.iter_mut() {
        moving.progress += (TEMPORARY_MOVE_SPEED / HEX_RADIUS_IN_METERS * 1000.0 * time.delta_secs() *60.0 / 3600.0)* PROGRESS_COMPLETE;
        ...
        if moving.progress >= PROGRESS_COMPLETE {
            ...
            if let Some(next) = path.waypoints.iter().skip_while(|&&h| h != moving.destination).nth(1).copied() {
